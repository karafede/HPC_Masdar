...
#######################################################################################
    ...
    date <- str_sub(filenames_AQUA, start = 19, end = -5)
...
# PM25_2015_GRW <- crop(PM25_2015_GRW, extent(shp_WRF))
# PM25_2015_GRW <- mask(PM25_2015_GRW, shp_WRF)
...
# Data_Emirates_tif_1km <- crop(Data_Emirates_tif_1km, extent(shp_WRF))
# Data_Emirates_tif_1km <- mask(Data_Emirates_tif_1km, shp_WRF)
...
map <- leaflet() %>%
  ...
  addLegend("bottomright", pal = rast_pal_EMIRATES, values = values(Emirates_tif_1km_tiff),# values = c(MIN_data, MAX_data),
          #  title = paste("<br><strong>AOD (MODIS-1km)",folder_day,"</strong>"),
